...
def train_epoch(nn_dict,train_loader,params,diffusion_process,optimizer,scheduler=None,ema=None,ema_model=None):
    egnn = nn_dict['egnn']
    egnn.train()
    egnn.to('cuda')
    ...
    for train_data in train_loader:
        ...
        if conditional:
            if params['NN_encoder']['spectrum_to_latent']:
                ...
                if torch.rand(1).item() < params['classifier_free']['dropout_rate']:
                    h_for_input = torch.cat((h_for_input,torch.zeros_like(train_data.spectrum.to('cuda'))),dim=1).to('cuda')
                else:
                    h_for_input = torch.cat((h_for_input,train_data.spectrum.to('cuda')),dim=1).to('cuda')
            else:
                h_for_input = torch.cat((h_for_input,train_data.spectrum.to('cuda')),dim=1).to('cuda')
        if params['give_excited_atom']:
            h_for_input = torch.cat((h_for_input,train_data.excited_atom.to('cuda')),dim=1)
        ...
        """
        epsilon_h, epsilon_x = egnn(train_data.edge_index.to('cuda'),h_for_input,train_data.pos_at_t.to('cuda'))
        epsilon_h = epsilon_h[:,:atom_type_size] #atom_typeの部分のみを取り出す
        """
        ...
        #loss = get_exact_loss(predicted_epsilon,target_epsilon,diffusion_process,train_data.each_time_list.squeeze(),train_data)
...
def eval_epoch(nn_dict,eval_loader,params,diffusion_process,optimizer,scheduler=None,ema=None,ema_model=None):
    ...
    with torch.no_grad():
        for val_data in eval_loader:
            ...
            if params['give_excited_atom']:
                h_for_input = torch.cat((h_for_input,val_data.excited_atom.to('cuda')),dim=1)
            h_for_input = torch.cat((h_for_input,time_data.to('cuda')),dim=1)
            ...
            epsilon_h = h[:,:atom_type_size] #atom_typeの部分のみを取り出す
            """
            epsilon_h, epsilon_x = egnn(val_data.edge_index.to('cuda'),h_for_input,val_data.pos_at_t.to('cuda'))
            epsilon_h = epsilon_h[:,:atom_type_size]
            """
            #lossの計算
            ...
            loss = get_loss(predicted_epsilon,target_epsilon,graph_index)
            #loss = get_exact_loss(predicted_epsilon,target_epsilon,diffusion_process,val_data.each_time_list.squeeze(),val_data)
...
def generate(nn_dict,test_data,params,diffusion_process,gen_num_per_spectrum=5,ema=None,ema_model=None,check=False):
    #モデルの読み込み
    if params['EMA']['use_ema']:
        egnn = ema_model
    else:
    ...
    with torch.no_grad():
        for i in tqdm(range(len(test_data))):
            ...
            while num_of_generated_data != how_many_gen:
                ...
                #time_list = list(range(1,30,1)) + list(range(30,300,3)) + list(range(300,500,5)) + list(range(500, num_diffusion_timestep+1, 10))
                #time_list.reverse()
                ...
                for t in time_list:
                    ...
                    if params["classifier_free"]["use_classifier_free"]:
                        ...
                        #無条件特徴量ベクトルを用いて予測
                        new_unconditional_h, new_unconditional_x = egnn(graph.edge_index,unconditional_h,graph.pos)
                        epsilon_unconditional_x = new_unconditional_x - graph.pos
                        epsilon_unconditional_x = remove_mean(epsilon_unconditional_x)
                        epsilon_unconditional_h = new_unconditional_h[:,:atom_type_size]
                    ...
                    if not torch.isfinite(graph.x).all():
                        ...
                        #seed_value += 1
                        if num_of_generated_nan == how_many_gen*2:
