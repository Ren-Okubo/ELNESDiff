...
def train_epoch(nn_dict,train_loader,params,diffusion_process,optimizer,scheduler=None,ema=None,ema_model=None):
    ...
    for train_data in train_loader:
        ...
        h, x = egnn(train_data.edge_index.to(device),h_for_input,train_data.pos_at_t.to(device))
...
def eval_epoch(nn_dict,eval_loader,params,diffusion_process,optimizer,scheduler=None,ema=None,ema_model=None):
    ...
    with torch.no_grad():
        for val_data in eval_loader:
            ...
            #equivariant graph neural networkによる予測
            h, x = egnn(val_data.edge_index.to(device),h_for_input,val_data.pos_at_t.to(device))
            ...
            while num_of_generated_data != how_many_gen:
                ...
                for t in time_list:
                    ...
                    if not torch.isfinite(graph.x).all():
                        ...
                        if num_of_generated_nan == how_many_gen*2:
                            print(f'too much nan was generated. Skipping this generation process: {data.id}. num_atoms: {num_of_atoms}')
                            # nextの生成プロセスに移る
                            num_of_generated_data = how_many_gen
                            break
                        break
                ...
                if torch.isfinite(graph.x).all() and torch.isfinite(graph.pos).all():
                    ...
                    if torch.any(graph.pos > 1000):
                        print('unrealistic coordinate was generated')
                        continue
