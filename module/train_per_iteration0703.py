...
import wandb
from def_for_main import EdgeIndexJudge
...
def train_epoch(nn_dict,train_loader,params,diffusion_process,optimizer,scheduler=None,ema=None,ema_model=None,flag='normal'):
    ...
    for train_data in train_loader:
        ...
        train_data.edge_index = train_data.edge_index.to('cuda')
...
def eval_epoch(nn_dict,eval_loader,params,diffusion_process,optimizer,scheduler=None,ema=None,ema_model=None,flag='normal'):
    ...
    with torch.no_grad():
        for val_data in eval_loader:
            ...
            val_data.edge_index = val_data.edge_index.to('cuda')
    ...
    with torch.no_grad():
        for batch in tqdm(test_data):
            ...
            while num_of_generated_data != how_many_gen and num_of_generated_nan < how_many_gen:
                ...
                graph.num_graphs = 1
                graph.batch = torch.tensor([0 for i in range(num_of_atoms)],dtype=torch.long)
                ...
                for t in time_list:
                    ...
                    if flag == 'gemnet':
                        ...
                        graph = graph.to('cuda')
                        graph.each_time_list = torch.tensor([[t] for _ in range(time_tensor.shape[0])],dtype=torch.long).to('cuda')
