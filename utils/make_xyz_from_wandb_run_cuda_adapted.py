...
def write_xyz_for_abinitio(run_id,generated_abinitio_data_path):
    ...
    datalist = torch.load(generated_abinitio_data_path, map_location=device, weights_only=False)
    for i in range(len(datalist)):
        item_device = datalist[i][-1].x.device if hasattr(datalist[i][-1], 'x') else device
        data = datalist[i][-1].to(item_device)
        si_tensor = torch.tensor([0,1], device=item_device)
        o_tensor = torch.tensor([1,0], device=item_device)
        with open(os.path.join('/mnt/homenfsxx/rokubo/data/diffusion_model/xyz',run_id,f'{i}.xyz'),'w') as f:
            ...
            for j in range(data.pos.shape[0]):
                if torch.equal(data.x[j], si_tensor):
                    atom_type = 'Si'
                elif torch.equal(data.x[j], o_tensor):
                    atom_type = 'O'
...
def write_xyz(save_path,original_graph,generated_graph,comment=None):
    ...
    generated_graph = generated_graph.to(device)
...
if __name__ == '__main__':
    ...
    data = torch.load(rmsd_path, map_location=device, weights_only=False)
